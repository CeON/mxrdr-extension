...
import pl.edu.icm.pl.mxrdr.extension.importer.cif.file.internal.CifFileResultCreator;
import pl.edu.icm.pl.mxrdr.extension.importer.cif.file.internal.CifMetadataReader;
import pl.edu.icm.pl.mxrdr.extension.importer.cif.file.internal.ImportedValuesStorage;
...
public class CifFileImporter implements MetadataImporter {
    ...
    public List<ResultField> fetchMetadata(Map<ImporterFieldKey, Object> importerInput) {
        MmCifFile mmCifFile = CifFileReader.readAndParseCifFile(importerInput)
                .orElseGet(() -> new MmCifFile(Collections::emptyList));
        ImportedValuesStorage importedValuesStorage = new CifMetadataReader().readToResultFields(mmCifFile);
        return new CifFileResultCreator(importedValuesStorage).createResultFields();
    }
