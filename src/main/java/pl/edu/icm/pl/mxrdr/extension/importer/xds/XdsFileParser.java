...
public class XdsFileParser {
    ...
    /**
     * Extracts metadataFields from String lines taken from XDS output file (usually "CORRECT.LP").
    ...
    List<ResultField> parse(List<String> dataLines) {
        ...
        for (String dataLine : dataLines) {
            ...
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_DETECTOR_DISTANCE.getValue(),
                    "DETECTOR_DISTANCE", 0, dataLine, dataCollectionChildren, collectedFields);
            
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_OSCILLATION_STEP_SIZE.getValue(),
                    "OSCILLATION_RANGE", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_WAVE_LENGTH.getValue(),
                    "X-RAY_WAVELENGTH", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_NUMBER_OF_FRAMES.getValue(),
                    "DATA_RANGE", 1, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_DETECTOR_OVERLOAD.getValue(),
                    "OVERLOAD", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_ORG_X.getValue(),
                    "ORGX", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_ORG_Y.getValue(),
                    "ORGY", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_DETECTOR_THICKNESS.getValue(),
                    "SENSOR_THICKNESS", 0, dataLine, dataCollectionChildren, collectedFields);
            addExtractedValue(MxrdrMetadataField.DATA_COLLECTION_STARTING_ANGLE.getValue(),
                    "STARTING_ANGLE", 0, dataLine, dataCollectionChildren, collectedFields);
        ...
        if (!unitCellConstantsCollectionChildren.isEmpty()) {
            results.add(ResultField.of(MxrdrMetadataField.UNIT_CELL_PARAMETERS.getValue(), unitCellConstantsCollectionChildren.toArray(new ResultField[0])));
        }
        ...
        if (!overallCollectionChildren.isEmpty()) {
            results.add(ResultField.of(MxrdrMetadataField.OVERALL.getValue(), overallCollectionChildren.toArray(new ResultField[0])));
        }
        if (!hrsCollectionChildren.isEmpty()) {
            results.add(ResultField.of(MxrdrMetadataField.HRS.getValue(), hrsCollectionChildren.toArray(new ResultField[0])));
        }

        results.add(ResultField.of(MxrdrMetadataField.SPACE_GROUP.getValue(), ResultField.ofValue(mapSpaceGroup(spaceGroupNumber))));
    ...
    private boolean containsHrs(String dataLine) {
   		...
   		if (values.length == 14) {
   			...
   			if (resolution > 0.3 && resolution < 5.0
   				&& sigma > 1.19 && sigma <= 100
   				&& cc > 50.0 && values[4].endsWith("%")
   				&& values[5].endsWith("%") && values[6].endsWith("%")) {
   				return true;
    		}
    	}
    	return false;
    ...
    private void addHrsValues(String dataLine, String additionalDataLine, List<ResultField> results,
    	...
    	if (values.length == 14) {
    		...
    		results.add(ResultField.of(MxrdrMetadataField.HRS_DATA_RESOLUTION_RANGE_HIGH.getValue(), clearNonDigits(values[0])));
    		overallResults.add(ResultField.of(MxrdrMetadataField.OVERALL_DATA_RESOLUTION_RANGE_HIGH.getValue(), clearNonDigits(values[0])));
    		results.add(ResultField.of(MxrdrMetadataField.HRS_NUMBER_OBSERVED_REFLECTIONS.getValue(), clearNonDigits(values[1])));
    		results.add(ResultField.of(MxrdrMetadataField.HRS_NUMBER_UNIQUE_REFLECTIONS.getValue(), clearNonDigits(values[2])));
    		results.add(ResultField.of(MxrdrMetadataField.HRS_NUMBER_POSSIBLE_REFLECTIONS.getValue(), clearNonDigits(values[3])));
    		results.add(ResultField.of(MxrdrMetadataField.HRS_ANOMALOUS_CORRELATION.getValue(), clearNonDigits(values[11])));
    	...
    	if (values.length == 14) {
    		results.add(ResultField.of(MxrdrMetadataField.HRS_DATA_RESOLUTION_RANGE_LOW.getValue(), clearNonDigits(additionalValues[0])));
    	}
    }

    private void addOverallValues(String dataLine, List<ResultField> results,
			String overallDataResolutionRangeLow) {
    	String[] values = StringUtils.normalizeSpace(dataLine).split(" ");
    	if (values.length == 14) {
    		...
    		results.add(ResultField.of(MxrdrMetadataField.OVERALL_NUMBER_OBSERVED_REFLECTIONS.getValue(), clearNonDigits(values[1])));
    		results.add(ResultField.of(MxrdrMetadataField.OVERALL_NUMBER_UNIQUE_REFLECTIONS.getValue(), clearNonDigits(values[2])));
    		results.add(ResultField.of(MxrdrMetadataField.OVERALL_NUMBER_POSSIBLE_REFLECTIONS.getValue(), clearNonDigits(values[3])));
    		results.add(ResultField.of(MxrdrMetadataField.OVERALL_ANOMALOUS_CORRELATION.getValue(), clearNonDigits(values[11])));
    		results.add(ResultField.of(MxrdrMetadataField.OVERALL_ANOMALOUS_SIGNAL.getValue(), clearNonDigits(values[12])));
    		if (StringUtils.isNotEmpty(overallDataResolutionRangeLow)) {
        		results.add(ResultField.of(MxrdrMetadataField.OVERALL_DATA_RESOLUTION_RANGE_LOW.getValue(), clearNonDigits(overallDataResolutionRangeLow)));
    ...
    public void addExtractedValue(String name, String sourceName, int sourcePosition, String dataline, List<ResultField> results, List<String> collected) {
